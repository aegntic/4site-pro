...
export class AELTDTemplateEngine {
  ...
  generateTemplateHTML(siteData: SiteData, template: AELTDTemplate): string {
    // Check if we have a custom HTML template file
    const customTemplates = [
      { id: 'liquid-metal-commerce', file: 'liquid-metal-commerce.html' },
      { id: 'motion-design-system', file: 'motion-design-system.html' },
      { id: 'glassmorphism-dashboard', file: 'glassmorphism-dashboard.html' },
      { id: 'cyberpunk-terminal', file: 'cyberpunk-terminal.html' }
    ];

    const customTemplate = customTemplates.find(t => 
      template.title.toLowerCase().includes(t.id.replace(/-/g, ' ')) ||
      template.design_style.toLowerCase().includes(t.id.replace(/-/g, ' '))
    );

    if (customTemplate) {
      return this.loadCustomTemplate(customTemplate.file, siteData);
    }
   ...
   */
  private loadCustomTemplate(filename: string, siteData: SiteData): string {
    try {
      const templatePath = path.join(__dirname, '..', 'templates', filename);
      let template = fs.readFileSync(templatePath, 'utf-8');
      
      // Replace placeholders with actual data
      template = template.replace(/\{\{projectName\}\}/g, siteData.title);
      template = template.replace(/\{\{projectDescription\}\}/g, siteData.description);
      
      // Replace conditional descriptions
      template = template.replace(/\{\{projectDescription \|\| '.*?'\}\}/g, (match) => {
        return siteData.description || match.match(/'(.*?)'/)?.[1] || '';
      });
      
      return template;
    } catch (error) {
      console.error('Failed to load custom template:', error);
      return this.generateDefaultTemplate(siteData);
    }
  }
  ...
  private generateLiquidMetal(data: SiteData): string {
    return this.loadCustomTemplate('liquid-metal-commerce.html', data);
  }

  private generateDimensionalDashboard(data: SiteData): string {
    return this.loadCustomTemplate('glassmorphism-dashboard.html', data);
  }
  ...
  private generateCyberpunk(data: SiteData): string {
    return this.loadCustomTemplate('cyberpunk-terminal.html', data);
  }

  private generateMotionDesign(data: SiteData): string {
    return this.loadCustomTemplate('motion-design-system.html', data);
  }
